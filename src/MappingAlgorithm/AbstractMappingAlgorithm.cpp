#include "MappingAlgorithm/AbstractMappingAlgorithm.hpp"
#include <algorithm>
...
std::optional<Step> AbstractMappingAlgorithm::getForcedMove() const{
...
[[nodiscard]] Step AbstractMappingAlgorithm::nextStep() {
...
bool AbstractMappingAlgorithm::isFullyCharged() const
{
    return batteryMeter->getBatteryState() == maxBattery || batteryMeter->getBatteryState() >= maxSteps - stepsTaken; 
}
bool AbstractMappingAlgorithm::isOnCharger() const
{
    return relativeCoordinates == getChargerLocation();
}
uint32_t AbstractMappingAlgorithm::stepsUntilMustBeOnCharger(uint32_t offeset) const
{
    uint32_t battery = 0;
...
bool AbstractMappingAlgorithm::mustReturnToCharger() const
{
...
Step AbstractMappingAlgorithm::stepTowardsCharger() const
{
    auto [results,iterator]= noWallGraph.bfs_find_first(relativeCoordinates, std::function<bool (const Coordinate<int32_t> &, const BFSResult &)> (
        [&](const Coordinate<int32_t> &coordinate, const BFSResult &){
            return coordinate == getChargerLocation();
...
uint32_t AbstractMappingAlgorithm::getLengthToCharger(Coordinate<int32_t> from) const
{
...
bool AbstractMappingAlgorithm::isExistsMappedCleanableTile() const
{
...
bool AbstractMappingAlgorithm::isCompletelyMapped() const
{
...
bool AbstractMappingAlgorithm::isProgressPossibleTheoretically() const
{
    auto [results,iterator] = noWallGraph.bfs_find_first(getChargerLocation(), std::function<bool (const Coordinate<int32_t> &, const BFSResult &)> (
        [&](const Coordinate<int32_t> &coordinate, const BFSResult & bfsResult){
            auto locationMapping = noWallGraph.getVertex(coordinate);
...
bool AbstractMappingAlgorithm::isAtMaxSteps() const
{
    return stepsTaken >= maxSteps;
}
bool AbstractMappingAlgorithm::isKnownCleanableTile(const HouseLocationMapping &locationMapping, BFSResult result) const
{
...
bool AbstractMappingAlgorithm::isPotentiallyCleanableTile(const HouseLocationMapping &locationMapping, BFSResult result) const
{
    return locationMapping.getHouseLocation().getLocationType() == LocationType::UNKNOWN && (result.getDistance() <= maxCleanableDistance());
}
std::optional<Step> AbstractMappingAlgorithm::getStepTowardsClosestReachableUnknown() const
{
    auto [results,iterator] = noWallGraph.bfs_find_first(relativeCoordinates, std::function<bool (const Coordinate<int32_t> &, const BFSResult &)> (
        [&](const Coordinate<int32_t> &coordinate, const BFSResult &bfsResult){
            auto locationMapping = noWallGraph.getVertex(coordinate);
...
Step AbstractMappingAlgorithm::getStepTowardsDestination(const Coordinate<int32_t>& destination, const std::shared_ptr<std::unordered_map<Coordinate<int32_t>,BFSResult>> results) const
{
...
void AbstractMappingAlgorithm::mapCurrentLocation()
{
...
void AbstractMappingAlgorithm::mapDirection(Direction direction)
{
...
void AbstractMappingAlgorithm::updateLocationIfExists(const HouseLocation& newLocation)
{
    if (!noWallGraph.isVertex(relativeCoordinates))
    {
        return;
...
void AbstractMappingAlgorithm::mapSurroundings()
{
    mapCurrentLocation();
